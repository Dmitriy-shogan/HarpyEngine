#pragma once
#ifndef HARPY_UTILITIES_THREADING_POOL
#define HARPY_UTILITIES_THREADING_POOL
/*
 * Thanks https://github.com/DeveloperPaul123 for this amazing thread pool
...
namespace harpy::utilities::threading {
...
#ifdef HARPY_UTILITIES_THREADS_NUMBER_MAX
    inline constexpr uint thread_num = std::thread::hardware_concurrency();
#else
    inline constexpr uint thread_num = HARPY_UTILITIES_THREADS_NUMBER;
#endif
    
#else
    inline const uint32_t thread_num = std::thread::hardware_concurrency();
#endif //HARPY_UTILITIES_THREADS_NUMBER
    ...
    public:                                               
        ...
        static thread_pool& get_singleton()
        {
            static thread_pool<FunctionType, ThreadType> pool{};
            return pool;
        }
      ...
      private:
        thread_pool()
            ...
            for (std::size_t i = 0; i < thread_num; ++i) {
                priority_queue_.push_back(std::move(current_id));
                try {
...
#endif //HARPY_UTILITIES_THREADING_POOL