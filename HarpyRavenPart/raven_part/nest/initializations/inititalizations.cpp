...
//QUEUES INCLUDE
#include <nest/wrappers/queues/compute_family_queue.h>
#include <nest/wrappers/queues/graphics_family_queue.h>
#include <nest/wrappers/queues/universal_family_queue.h>
#include <nest/wrappers/queues/graphics_family_queue.h>
#include <nest/wrappers/queues/compute_family_queue.h>
#include <nest/wrappers/queues/transfer_family_queue.h>
#include <nest/wrappers/queues/main_family_queue.h>
...
void init_window()
{
    ...
    resource::get_resource().set_main_window(harpy::nest::windowing::window({800, 600, false, false, "Testing"}));
...
void init_vk_queues_and_logical_device()
{
    ...
    {
        logger::get_logger() << "Found more than one queue. Good.";
        unsigned flags{};
        ...
        for(uint32_t f = 0; auto& i : families)
        {
            ...
            {
                resource::get_resource().get_all_queues().push_back({wrappers::main_family_queue{f++}});
                flags |= wrappers::main;
                queue_amount_to_create.push_back(1);
                continue;
            }
            ...
            {
                resource::get_resource().get_all_queues().push_back({wrappers::graphics_family_queue{f++}});
            ...
            {
                resource::get_resource().get_all_queues().push_back({wrappers::compute_family_queue{f++, i.queueFamilyProperties.queueCount}});
            ...
            {
                resource::get_resource().get_all_queues().push_back({wrappers::transfer_family_queue{f++, i.queueFamilyProperties.queueCount}});
    ...
    for(int i = 0; i < families.size(); i++)
    {
        ...
        queues_ci[i].queueFamilyIndex = resource::get_resource().get_all_queues().front().get_family_index();
    ...
    logger::get_logger() << "Succesfully created logical device. Starting initilizing queues in resource";
    
    for(auto& i : resource::get_resource().get_all_queues())
        i.init();

    logger::get_logger() << "Succesfully initialized queues. Going to create surface";
...
void harpy::nest::initializations::init_vk_common_resource()
{
    ...
    init_vk_queues_and_logical_device();
    init_vk_surface();
    init_vma();
...
void initializations::init_std_surface_capabilities()
{
    ...
    vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
        ...
        resource::get_resource(),
        &std_surface_capabilities.capabilities);
    uint32_t format_count{};
        
    vkGetPhysicalDeviceSurfaceFormatsKHR(
        resource::get_resource(), //Physical device
        resource::get_resource(), //Surface
        &format_count,
        nullptr);
    ...
    for(auto& i : formats)
    {
        std_surface_capabilities.formats.emplace_back(i.format);
        std_surface_capabilities.color_spaces.emplace_back(i.colorSpace);
    }
    ...
    std_surface_capabilities.present_modes.resize(present_modes_count);
	
    vkGetPhysicalDeviceSurfacePresentModesKHR(
    resource::get_resource(), //Physical device
    resource::get_resource(), //Surface
    &present_modes_count,
    std_surface_capabilities.present_modes.data());
	
    //Stolen composite alpha thing. Don't know yet how to properly use if, needs research
    std_surface_capabilities.composite_alpha = VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR;
	
    if (std_surface_capabilities.composite_alpha == VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR)
    {
        if (std_surface_capabilities.capabilities.supportedCompositeAlpha & VK_COMPOSITE_ALPHA_INHERIT_BIT_KHR)
        {
            std_surface_capabilities.composite_alpha = VK_COMPOSITE_ALPHA_INHERIT_BIT_KHR;
        }
        else if (std_surface_capabilities.capabilities.supportedCompositeAlpha & VK_COMPOSITE_ALPHA_POST_MULTIPLIED_BIT_KHR)
        {
            std_surface_capabilities.composite_alpha = VK_COMPOSITE_ALPHA_POST_MULTIPLIED_BIT_KHR;
        }
        else if (std_surface_capabilities.capabilities.supportedCompositeAlpha & VK_COMPOSITE_ALPHA_PRE_MULTIPLIED_BIT_KHR)
        {
            std_surface_capabilities.composite_alpha = VK_COMPOSITE_ALPHA_PRE_MULTIPLIED_BIT_KHR;
        }
    }
}
